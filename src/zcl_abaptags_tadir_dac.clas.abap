"! <p class="shorttext synchronized" lang="en">DB Access for TADIR</p>
CLASS zcl_abaptags_tadir_dac DEFINITION
  ...
  PUBLIC SECTION.
    CLASS-METHODS:
      "! <p class="shorttext synchronized" lang="en">Returns current instance</p>
      get_instance
        RETURNING
          VALUE(result) TYPE REF TO zcl_abaptags_tadir_dac.

    METHODS:
      "! <p class="shorttext synchronized" lang="en">Retrieves tadir infos for given list of tadir keys</p>
      get_tadir_infos
        IMPORTING
  ...
  PRIVATE SECTION.
    CLASS-DATA:
      instance TYPE REF TO zcl_abaptags_tadir_dac.
ENDCLASS.
