CLASS zcl_abaptags_message_helper DEFINITION
  ...
  PRIVATE SECTION.
    TYPES ty_message TYPE c LENGTH 200.

    CONSTANTS c_length_of_msgv TYPE i VALUE 50.
    CONSTANTS c_offset_of_last_character TYPE i VALUE 49.
...
CLASS zcl_abaptags_message_helper IMPLEMENTATION.
  METHOD set_msg_vars_for_clike.
    split_text( EXPORTING text  = text
  ...
  METHOD split_text.
    ...
    DATA msg_var TYPE c LENGTH c_length_of_msgv.
    DATA rest TYPE ty_message.
    DATA index TYPE syst-index.
    ...
    DO 4 TIMES.
      ...
      CALL FUNCTION 'TEXT_SPLIT'
        EXPORTING length = c_length_of_msgv
                  text   = tmp_text
        IMPORTING line   = msg_var
                  rest   = rest.
